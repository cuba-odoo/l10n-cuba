...
class PurchaseCostDistributionLine(models.Model):
    ...
    def _get_product_qty(self):
        for dist_line in self:
            # Cannot be done via related
            #  field due to strange bug in update chain
            # TecKisa -distribucion de coste en base a cantidades que viene en PO
            dist_line.product_qty = dist_line.move_id.product_qty # Original
            # dist_line.product_qty = dist_line.move_id.ordered_qty   # TecKisa
...
class PurchaseCostDistributionExpense(models.Model):
    ...
    move_line = fields.Many2one(
        comodel_name='account.move.line', string="Supplier move line",
        domain="[('move_id.type', '=', 'in_move'),"
    ...
    def onchange_move_line(self):
        self.move_id = self.move_line.move_id.id
        self.expense_amount = self.move_line.price_subtotal
