...
class InventoryAdjustmentsGroup(models.Model):
    ...
    product_selection = fields.Selection(
        [
            ...
            ("percent", "Percent of products"),
        ],
        default="all",
        required=True,
    )

    product_percent = fields.Char(string="Percent products")
    ...
    def _get_quants(self, locations):
        ...
        elif self.product_selection == "percent":
            domain = self._get_domain_percent_quant(base_domain)
    ...
    def _get_domain_percent_quant(self, base_domain):
        _logger.debug(self.product_percent)

        percent = int(self.product_percent)
        products = self.env['product.product'].search(base_domain)
        _logger.debug(products.ids)
        qty = int(len(products.ids) * percent / 100)
        _logger.debug(qty)
        product_ids = random.choices(products.ids, k=qty)

        return expression.AND(
            [base_domain, [("product_id", "in", product_ids)]]
        )
