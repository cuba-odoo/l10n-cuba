...
class ReportPayslip(models.AbstractModel):
    ...
    def _get_line_by_code(self, payslip_run, code):
        return _get_line_by_code(self, payslip_run, code)

    def _get_slip(self, payslip_run):
        """Obtienen las nominas de un pago masivo."""
        domain = [('payslip_run_id', '=', payslip_run.id)]
    ...
    def _get_rules_by_cat_code_header(self, payslip_run_id, code_arg):
        #        Usado para crear las cabeceras de las tablas.
        query = """SELECT sr.code, sr.name
                FROM hr_payslip_run pr inner join hr_payslip ps on (ps.payslip_run_id = pr.id)
                ...
                ORDER BY pl.sequence""" % (payslip_run_id, code_arg)

        self.env.cr.execute(query)
        result = self.env.cr.dictfetchall()
        return result

    def _get_amount_by_cat_code(self, payslip_run_id, code_arg, emp):
        ...
        query = """SELECT sr.id, sr.code, sr.name, sr.appears_on_payslip, pl.sequence, pl.amount
                    ...
                    ORDER BY pl.sequence""" % (payslip_run_id, code_arg, emp)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        report = report_obj._get_report_from_name('l10n_cu_hr_payroll.report_payslip')
